...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!clickType || !['github', 'linkedin', 'instagram'].includes(clickType)) {
      return NextResponse.json(
    ...
    // Increment the specific click count using the redis client
    const clicks = await redis.incr(redisKey);
